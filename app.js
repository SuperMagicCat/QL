...
async function loadEntries() {
  try {
    ...
    if (!Array.isArray(payload)) throw new Error("entries.json must contain an array");
    entries = payload;
    renderCategories();
    renderEntries();
  } catch (error) {
    console.error("Unable to load compendium entries", error);
    resultsTitle.textContent = "资料读取失败";
    resultCount.textContent = "";
    entryGrid.innerHTML = "";
    emptyState.hidden = false;
    emptyState.querySelector("h3").textContent = "资料暂时无法读取";
    emptyState.querySelector("p").textContent = "请稍后刷新页面，或检查 data/entries.json 是否存在。";
